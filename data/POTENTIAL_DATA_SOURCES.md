...
| **REMOVED** (Xu/Zhao 2024, GENE; github.com/REMOVED/REMOVED) | code repo; `data/testdata/{katoh,reynold}.csv` (662+253 rows, current tree) + `data/ExperimentalData/Covid19.csv` (deleted from `main` in commit `d2ad931`, still recoverable from parent commit `f3254ee`) | ⛔ nothing further | katoh.csv: 662/662 already present. reynold.csv: 240/253 already present, remaining 12 have no gene-identity column. Covid19.csv (recovered from git history): 11 candidate sequences but no efficacy/label column in the committed version. (This repo's REMOVED_8.csv panel was integrated and later removed — no license covered it.) |
| **Matveeva et al. 2007** (3,336-siRNA compilation) | large compilation | ⛔ | only download link dead, no Wayback, 3/4 sources paywalled |
| **Ichihara et al. 2007 i-Score supplement** (NAR 35(18):e123, CC BY-NC 2.0 UK) | supplementary workbook tabulating every classic dataset the i-Score model used, with a per-row `Authors` column: Huesken, Reynolds, Vickers, Harborth, Ui-Tei, Khvorova, Takasaki | 🟢 44 Harborth rows integrated | **The route to paywalled classic datasets**: Harborth et al. 2003 is not open access, but this open-access paper republishes its 44 lamin-A/C rows with sequences and values. The same workbook also carries the other classic sets — most already in this corpus via siRNAEfficacyDB, but it is the obvious place to look when another of them turns out to be missing or corrupt upstream (as siRNAEfficacyDB's Lamin A block was). |
| **Sciabola et al. 2013 in-house panel** (NAR 41(3):1383, Supp. Tables S3/S4) | the paper's OWN Hep3B/QuantiGene 2.0 screen: 361 21-mers across 10 HCC-relevant genes, % inhibition at up to 4 doses, plus 591 R-/L-Dicer substrates | 🟢 356/361 21-mers integrated (CC BY-NC 3.0) | Obtained from PMC3561943's supplementary Word file (Oxford's own CDN 403s). Table S3 mixes strand orientations — every row verified in both orientations against fresh RefSeq. 5 rows dropped as unlocatable in any checked transcript. The 591 Dicer substrates are still unintegrated — a different duplex architecture, and the obvious next increment from this file. |
...
6. **Licensing** — verify redistribution terms (Model B's prepared CSVs, e.g., are proprietary-licensed and were deliberately not vendored).
