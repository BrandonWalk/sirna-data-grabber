...
| **REMOVED** (Xu/Zhao 2024, GENE; github.com/REMOVED/REMOVED) | code repo; `data/testdata/{katoh,reynold}.csv` (662+253 rows, current tree) + `data/ExperimentalData/{Covid19,REMOVED_8}.csv` (deleted from `main` in commit `d2ad931`, still recoverable from parent commit `f3254ee`) | ✅ REMOVED_8 integrated, rest redundant/unusable | katoh.csv: 662/662 already present. reynold.csv: 240/253 already present, remaining 12 have no gene-identity column. Covid19.csv (recovered from git history): 11 candidate sequences but no efficacy/label column in the committed version. **REMOVED_8.csv (recovered from git history): 8 real siRNAs against `REMOVED`, verified against NM_005018.3, integrated** — +1 gene (97→98). License unresolved (no LICENSE file in source repo); committed as `REMOVED` at the user's explicit request despite this gap — see `data/DATA_SOURCES.md` and `NOTICE.md`. |
